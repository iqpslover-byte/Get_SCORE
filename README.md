...
1. **予測** … Get_LAUNCHES の予定打上げ × Get_NAVWARN の警報を突合（anchor 200km＋日付±3日）。
   ...
   **v2 (2026-07-16)**: デブリ警報（SPACE DEBRIS）を「日付集合一致＋窓開始が打上げ警報の−30分〜+6h」で
   同一ミッションにペアリング。区域が緯度の折り返し（頂点）を含み最遠>10,000kmの場合は
   **頂点緯度法**（最大|緯度|≒傾斜角・`incl_method:"apex"`）を採用（Starship等の弾道飛行で
   重心方位法の+5°外れを+0.4°に改善）。コリドー折返しの誤反転・遠方区域の重心経度平均も修正
   （アプリ v3.34.2/.5〜.7 と同一判定）。
